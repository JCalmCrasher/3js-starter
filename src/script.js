...
material2.color = new THREE.Color(0x11bbff);
...
light1.add(pointLight.position, "x").min(-6).max(20).step(0.01);
...
// Light 3
const pointLight3 = new THREE.PointLight(0x00ff00, 0.2);
...
const tick = () => {
 ...
 box.rotation.y = 0.3 * elapsedTime;
 box.rotation.y += 0.5 * (targetX - box.rotation.y);
 box.rotation.x += 0.05 * (targetY - box.rotation.x);
 box.position.z += -0.05 * (targetY - box.rotation.x);
